...
var AuxiliarControler = require('../../dbQueries/auxiliarQueries')

// Auxiliar Route

router.get('/',passport.authenticate('jwt',{session: false}), async (req,res,next) => {
    var auxiliares = await AuxiliarControler.getAllAuxiliares()
...
router.get('/ativos',passport.authenticate('jwtAdmin',{session: false}), async (req,res,next) => {
    var auxiliares = await AuxiliarControler.getAuxiliaresByEstado(1);
    res.status(200).send(auxiliares)
})


router.get('/inativos',passport.authenticate('jwtAdmin',{session: false}), async (req,res,next) => {
    var auxiliares = await AuxiliarControler.getAuxiliaresByEstado(0);
    res.status(200).send(auxiliares)
})

router.get('/admins',passport.authenticate('jwtAdmin',{session: false}), async (req,res,next) => {
    var auxiliares = await AuxiliarControler.getAuxiliaresByEstado(2);
    res.status(200).send(auxiliares)
})
...
router.post('/criar',passport.authenticate('jwtAdmin',{session: false}), async (req,res,next) => {
    ...
    var auxiliar = await AuxiliarControler.addAuxiliar(id,password,contacto,nome,apelido,data_nascimento,rua,localidade,codigo_postal,cidade,estado);
    res.status(200).send(auxiliar)
})

router.get('/:uid',passport.authenticate('jwtAdmin',{session: false}), async (req,res,next) => {
    var auxiliar = await AuxiliarControler.getAuxiliarById(req.params.uid);
    res.status(200).send(auxiliar)
})

router.get('/desativar/:uid',passport.authenticate('jwtAdmin',{session: false}), async (req,res,next) => {
    var auxiliar = await AuxiliarControler.mudarEstadoAuxiliarById(req.params.uid, 0);
    res.status(200).send(auxiliar)
})

router.get('/ativar/:uid',passport.authenticate('jwtAdmin',{session: false}), async (req,res,next) => {
    var auxiliar = await AuxiliarControler.mudarEstadoAuxiliarById(req.params.uid, 1);
    res.status(200).send(auxiliar)
})

router.get('/admin/:uid',passport.authenticate('jwtAdmin',{session: false}), async (req,res,next) => {
    var auxiliar = await AuxiliarControler.mudarEstadoAuxiliarById(req.params.uid, 2);
    res.status(200).send(auxiliar)
})
